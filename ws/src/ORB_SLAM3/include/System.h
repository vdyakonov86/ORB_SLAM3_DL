...
public:
    ...
    // Returns the camera pose (empty if tracking fails).
    Sophus::SE3f TrackRGBD(std::vector<OutputSeg> &seg_result,const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas = vector<IMU::Point>(), string filename="");
