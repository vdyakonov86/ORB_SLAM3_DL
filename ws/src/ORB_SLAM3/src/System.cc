...
Sophus::SE3f System::TrackRGBD(std::vector<OutputSeg> &seg_result,const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageRGBD(seg_result,imToFeed,imDepthToFeed,timestamp,filename);
